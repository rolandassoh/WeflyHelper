...
public class CultureTypesGetTask extends AsyncTask<Void, Integer, Boolean> {
    private DBActivity act;
    String response = "";
    private OnCultureTypesDownloadCompleteListener listener;
    private String TAG = getClass().getSimpleName();
    private boolean mustStop;
    private AppController appController;

    public CultureTypesGetTask(@NonNull DBActivity activity){
        this.act = activity;
    ...
    protected Boolean doInBackground(Void... voids) {
        try {
            ...
            response = util.getResponseFromHttpUrl(Constants.CULTURE_TYPE_URL);
    ...
    protected void onPostExecute(Boolean isOk) {
        if(isCancelled())
            return;
        try {
            mustStop = Save.defaultLoadBoolean(Constants.PRESENTER_LOADER_STOP_TASK_IS_ENABLE, act);
            if (!mustStop)
                notifyOnCultureTypesDownloadCompleteListener(isOk, response);
        }catch (Exception e){
            e.printStackTrace();
        }
    }

    public void setOnCultureTypesDownloadCompleteListener(@NonNull OnCultureTypesDownloadCompleteListener listener) {
        this.listener = listener;
    }

    public static interface OnCultureTypesDownloadCompleteListener {
        void onDownloadError(@NonNull String errorMsg);
        void onDownloadSucces(@NonNull JSONArray cultTypeJArray);
    }

    private void notifyOnCultureTypesDownloadCompleteListener(boolean isOK, @NonNull String response) {
        if (listener != null){
            try {
                if (isOK){
